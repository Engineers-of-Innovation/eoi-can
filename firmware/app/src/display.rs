//! Everything the display boards share: the panel bring-up, the refresh loop,
//! the e-paper draw target, the CAN collector plumbing, and the bxCAN interrupt
//! workarounds.
//!
...
use embassy_stm32::can::{
    ...
    RxBuf, SceInterruptHandler, TxBuf, TxInterruptHandler, filter::Mask32,
};
use embassy_stm32::gpio::{Input, Level, Output, Pull, Speed};
use embassy_stm32::interrupt::typelevel::{
    Binding, CAN1_RX0, CAN1_RX1, CAN1_SCE, CAN1_TX, DMA1_CHANNEL4, DMA1_CHANNEL5,
};
use embassy_stm32::peripherals::{CAN1, DMA1_CH4, DMA1_CH5, IWDG};
...
/// Interrupts a display board's `bind_interrupts!` has to cover.
///
/// Spelled out as a bound rather than declared here because `bind_interrupts!`
/// emits `#[no_mangle]` handlers: one copy in this library would collide at link
/// time with the `CAN1_*` handlers the rudder and height-sensor binaries declare
/// for themselves.
pub trait DisplayIrqs:
    Binding<CAN1_TX, TxInterruptHandler<CAN1>>
    + Binding<CAN1_RX0, Rx0InterruptHandler<CAN1>>
    + Binding<CAN1_RX1, Rx1InterruptHandler<CAN1>>
    + Binding<CAN1_SCE, SceInterruptHandler<CAN1>>
    + Binding<DMA1_CHANNEL4, dma::InterruptHandler<DMA1_CH4>>
    + Binding<DMA1_CHANNEL5, dma::InterruptHandler<DMA1_CH5>>
    + Copy
    + 'static
{
}

impl<T> DisplayIrqs for T where
    T: Binding<CAN1_TX, TxInterruptHandler<CAN1>>
        + Binding<CAN1_RX0, Rx0InterruptHandler<CAN1>>
        + Binding<CAN1_RX1, Rx1InterruptHandler<CAN1>>
        + Binding<CAN1_SCE, SceInterruptHandler<CAN1>>
        + Binding<DMA1_CHANNEL4, dma::InterruptHandler<DMA1_CH4>>
...
pub async fn run_display<I: DisplayIrqs>(
    ...
    // which is why the display boards have no I2C temperature sensor.
    ...
    // A display board originates no traffic; the TX half exists only so it can
