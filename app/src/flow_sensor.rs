...
// NTC voltage divider: 47 kΩ to VREF on top, NTC to GND on bottom.
// NTC: 50 kΩ at 25 °C, B = 3950 K.
const NTC_TOP_OHMS: f32 = 47_000.0;
const NTC_R0_OHMS: f32 = 50_000.0;
const NTC_T0_K: f32 = 298.15;
const NTC_B_K: f32 = 3950.0;
...
type Adc2Mutex = Mutex<CriticalSectionRawMutex, Adc<'static, ADC2>>;
static ADC2_CELL: StaticCell<Adc2Mutex> = StaticCell::new();

pub struct FlowSensorPeripherals {
    pub tim_in: LowLevelTimer<'static, TIM2>,
    pub tim_out: LowLevelTimer<'static, TIM15>,
    pub ntc_in: Peri<'static, PA1>,
    pub ntc_out: Peri<'static, PA3>,
    pub adc: &'static Adc2Mutex,
}

pub fn init(
    tim2: Peri<'static, TIM2>,
    pulse_in: Peri<'static, PA0>,
    pulse_out: Peri<'static, PA2>,
    adc2: Peri<'static, ADC2>,
    ntc_in: Peri<'static, PA1>,
    ntc_out: Peri<'static, PA3>,
) -> FlowSensorPeripherals {
    ...
    let tim_in = setup_pulse_counter(tim2, pulse_in);
    let tim_out = setup_pulse_counter(tim15, pulse_out);
    let adc = ADC2_CELL.init(Mutex::new(Adc::new(adc2)));
    FlowSensorPeripherals {
        tim_in,
        tim_out,
        ntc_in,
...
fn raw_to_centidegrees(raw: u16) -> i16 {
    if raw == 0 || raw >= ADC_MAX {
        return TEMP_INVALID_CDEG;
    }
    let r_ntc = NTC_TOP_OHMS * (raw as f32) / ((ADC_MAX - raw) as f32);
    let inv_t = 1.0 / NTC_T0_K + logf(r_ntc / NTC_R0_OHMS) / NTC_B_K;
